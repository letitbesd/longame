package com.longame.display.core
{
	...
	/**
	 * 用Clip可以人工生成一个在flash里面神通广大的MovieClip，更重要的是它可以实现位图渲染
	 * Clip的可能的使用场合：
	 * 1. 将一堆图片或显示对象组合成一个MovieClip
	 * 2. 将一个传统的MovieClip转换成一个Clip,并可以选择位图渲染，提高效率；
	 * 3. 可以做一个非常方便的导航器或ViewStack,比如作为ScreenManager，将Screen作为Clip的帧，这样是不是很方便？
	 * 4. 是不是可以用来做Preloader,SimpleButton,等等
	 * 思考：使用buildFromMovieClip，renderAsBitmap开启反而会消耗更多的的CPU和内存，位图渲染的优势在哪里？
	 * 问题： 由于程序运行的最小间隔就是EnterFrame,所以frameRate目前无法实践，frameRate只能是当前stage.frameRate的整数分之一，做个参数来搞这个？
	 */
	
	public class Clip extends Sprite implements IDestroyable, IAnimatedObject, IBitmapRenderer
	{
		 ...
		 */
		public function Clip(frameRate:int = 30) 
		{
		...
		/**
		 * todo，当从舞台删除时，自动stop，否则在特效的场合，会不停的运转，耗费资源
		 * 这个类要改，在一个entity被删除后，enity并没有调用destroy方法，能否在其owner销毁时，判断这个entity是否有owner，如果没有，也自动销毁它？
		 * */
		 ...
		 * @param source: MovieClip类名或类或实例
		 * @param renderAsBitmap: 是否采用位图渲染，否则Clip只是控制这个MovieClip而已,
		 *                        好处是控制比普通的MovieClip更加灵活,如gotoAndPlay("Roll",3),能将Roll动作播放3次
		 * */
		protected var mcSource:*;
		private var _mcBuilded:Boolean;
		public function buildFromMovieClip(source:*,renderAsBitmap:Boolean=false):void
		{
			if(this.mcSource==source) return;
			this.mcSource=source;
			this._mcBuilded=false;
//			this.reset();
            this.destroyRealMc();
			if(source is String){
				var cls:Class=AssetsLibrary.getClass(source as String);
				if(cls==null){
					RenderManager.getDisplayFromSource(source,
						function onGot(d:DisplayObject):void{
							if((d as MovieClip)==null){
								trace("bug 点");
							}
							doBuild(d as MovieClip,source,renderAsBitmap);
						},
			...
			if(source is Class){
				this.doBuild(new source() as MovieClip,source,renderAsBitmap);
			}
			else if(source is MovieClip){
				this.doBuild(source as MovieClip,source,renderAsBitmap);
			}else{
				throw new Error("source can only be MovieClip Class or Class name!");
			}
		}
		protected function doBuild(mc:MovieClip,source:*,renderAsBitmap:Boolean):void
		{
			this.reset();
			_realMc=mc;
			_renderAsBitmap=!renderAsBitmap;
			this.renderAsBitmap=renderAsBitmap;
			var fn:uint=_realMc.totalFrames;
			var label:String;
			for(_i=1;_i<=fn;_i++){
				label=MovieClipUtil.getLabel(_realMc,_i);
				this.addFrame(source,_i,label);
			}
			if(this._onBuild) this._onBuild.dispatch(this);
			_destroyed=false;
			_mcBuilded=true;
		}
		//////////////////////////////////////////////////
		 ...
		 */
		public function gotoAndPlay(frame:Object,loops:int=0,frameRate:int=-1):Boolean
		{
			if (_frames.length > 1)
			{
				if(frameRate<=0) frameRate=this.frameRate;
				convertToFrameIndex(frame);
				_playDirectionFlag = PLAY;
				gotoAndStopBase(_fn);
				playBaseFunction(_fn,_autoLoop);
				if((frame is String)&&(loops!=0)){
					labelToLoop.init(String(frame),_currentFrame,loops,frameRate);
				}else{
					labelToLoop.reset();
				}
			}
		 ...
		 * @param frameRate:播放帧频
		 */
		public function gotoAndReversePlay(frame:Object,frameRate:int=-1):void
		{
			if (_frames.length > 1)
			{
				this.frameRate=frameRate;
		 ...
		 * 
		 */
		public function play(autoLoop:Boolean = true, frameRate:int =-1):void
		{
			if (_frames.length > 1)
			{
				this.frameRate=frameRate;
		...
		public function destroy():void
		{
			...
			this.destroyBitmap();
			this.destroyRealMc();
			if(this._onDestroy) this._onDestroy.dispatch(this);
		}
		public function onFrame(deltaTime:Number):void
		{
//			if(this._frames.length==0) return;
//			if(ProcessManager.platformTime - _lastUpdateTime >= this._frameInterval)
//			{
//				this.handleFrame();
//				_lastUpdateTime = ProcessManager.platformTime;
//			}
			this.handleFrame();
		}
		...
		protected function stopRender():void
		{
			...
			if(this._onStop) this._onStop.dispatch(this);
		}
		/**
		 * gotoAndStopBase
		 * @param	frame
			...
			if (frameCheck(_fn))
			{
				...
				if (_frames[_fn - 1] != null)
				{
					...
					if(this._renderAsBitmap){
						RenderManager.render(this,_frameObject,effects,(_realMc?_fn:null));
					//如果非位图渲染，而_movieClipSource存在，则只需控制这个MovieClip即可
					}else if(_realMc){
						_realMc.gotoAndStop(_fn);
					}else{
						//删掉上一帧的对象
...
//					 Logger.error(this,"convertToFrameIndex","There is no label named :[ " + frame + " ]");
				}
				_fn=_i+1;
				return _fn;
			}
...
//			if(this._onLoop) this._onLoop.dispatch();
		...
		public function set renderAsBitmap(value:Boolean):void{
			...
			if(value){
				...
				this.destroyRealMc(false);		
			}else{
				if(_realMc){
					this.addChildAt(_realMc,0);
					_realMc.stop();
				}
				this.destroyBitmap();
				this.mcSource=null;
		...
		private function destroyRealMc(complete:Boolean=true):void
		{
			if(_realMc&&this.contains(_realMc)) this.removeChild(_realMc);
			if(complete) _realMc=null;
		}
		...
		public function get mcBuilded():Boolean
		{
			return _mcBuilded;
		}
		/**
		...
		{
			if(this._onPlay==null) _onPlay=new Signal(Clip);
			return _onPlay;
		}
		/**
		 * 停止播放事件
		...
		{
			if(this._onStop==null) _onStop=new Signal(Clip);
			return _onStop;
		}
		/**
		 * 每帧变化事件
		...
		{
			if(this._onFrameChange==null) _onFrameChange=new Signal(Clip);
		...
		{
			if(this._onLooped==null) _onLooped=new Signal(Clip);
			return _onLooped;
		}
		/**
		 * mc构建好了
		...
		{
			if(this._onBuild==null) _onBuild=new Signal(Clip);
			return _onBuild;
		}
		/**
		 * 被销毁
		...
		{
			if(this._onDestroy==null) _onDestroy=new Signal(Clip);
			return _onDestroy;
		}
	}
}
internal class LoopedLabelFrame
{
	...
	public function init(label:String,frame:uint,loop:int,frameRate:int):void
	{
		this.label=label;
		this.frame=frame;
		this.loops=loop;
