...
package com.xingcloud.util
{
	...
	public class Debug
	{
		 ...
		 * default: 'XingCloud';
		 */
		public static var password:String='XingCloud';
		...
		/**
		 *log trace location,either "console" or "standalone"
		 */
		public static var outputLocation:String="console";
		/**
		 * Status event
...
//		private static const OBJECT_OPERATION:String='debugObject';
		 ...
		 */
		public static function log(message:*, target:Object=null):Boolean
		{
			if (allowLog && showLog)
				return send(LOG_OPERATION, "[log]" + generationMessage(message, target), 0xFEFEFE);
			else
				return false;
		}
		 ...
		 * @return				True if successful
		 */
		public static function error(message:*, target:Object=null):Boolean
		{
			if (allowLog && showError)
				return send(ERROR_OPERATION, "[error]" + generationMessage(message, target), 0xCC0000);
			else
				return false;
		}
		 ...
		 * @return				True if successful
		 */
		public static function warning(message:*, target:Object=null):Boolean
		{
			if (allowLog && showWarning)
				return send(WARNING_OPERATION, "[warning]" + generationMessage(message, target), 0xCCCC00);
			else
				return false;
		}
...
//		}
		...
		private static function generationMessage(msg:*, target:Object):String
		{
			if (Reflection.isSimple(msg))
			{
				return timestamp() + targetName(target) + msg.toString();
			}
			else
			{
