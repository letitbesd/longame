package com.longame.modules.entities
{
	...
	import com.longame.modules.core.signals.SignalBus;
	...
	public class DisplayEntity extends AbstractEntity implements IDisplayEntity
	{
		...
		override protected function doWhenActive():void
		{
			...
			if(_scene&&(!(this is IDisplayGroup))){
				_scene.map.addChild(this);
			}
			//如果父亲是要自动深度排序的，将其添加到map中
			if(parent&&(parent!=_scene)&&parent.autoLayout) this.parent.map.addChild(this);
			super.doWhenActive();
			...
			if(_scene){
				...
				if(this._width==-1) this.width=_scene.tileSize;
				if(this._length==-1) this.length=_scene.tileSize;
				if(this._height==-1) this.height=_scene.tileSize;
			}
		}
		override protected function doWhenDeactive():void
		{
			...
			if(_scene&&(!(this is IDisplayGroup))){
				_scene.map.removeChild(this);
			}
			//如果父亲是要自动深度排序的，将其从map中删除
			if(parent&&(parent!=_scene)&&parent.autoLayout) this.parent.map.removeChild(this);
			_scene=null;
		...
		long_internal function notifyTileMove(delta:Point):void
		{
			...
			if(_scene&&(!(this is IDisplayGroup)))
			{
				_scene.map.moveChild(this,delta);
			}
			if(parent&&(parent!=_scene)&&parent.autoLayout) this.parent.map.moveChild(this,delta);
		...
		long_internal function notifyTileBoundsChange():void
		{
			//如果是单个显示个体，将其在scene中的map中更新
			if(_scene&&(!(this is IDisplayGroup))){
				_scene.map.changeChild(this);
			}
			if(parent&&(parent!=_scene)&&parent.autoLayout) this.parent.map.changeChild(this);
		...
		{
			if(_onMouse==null) _onMouse=new MouseSignals(this);
			return _onMouse;
		}
...
//				p.mouseEnabled=true;
		...
		{
			if(_onTileBoundsChange==null) _onTileBoundsChange=new Signal(IDisplayEntity);
