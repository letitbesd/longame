package com.longame.game.scene
{
	...
	public class Camera extends AbstractComp
	{
		...
		public var _transformInvalidated:Boolean;
		...
		* */
		...
		{
			if(_transformTween&&!_transformTween.paused) return false;
		...
		public var _transformTween:GTween;
...
//				child.container.x+=deltaX;
//				child.container.y+=deltaY;
				//转换成场景坐标
...
//				if(duration>=0) this.duration=duration;
		...
		{			
			var lensCenterInTargetParent:Point=sceneContainer.parent.globalToLocal(lensCenter);
			//考虑target的注册点，计算target实际应该移动到的位置
			var xx:Number=lensCenterInTargetParent.x-(_x+offsetX)*_zoom;
			var yy:Number=lensCenterInTargetParent.y-(_y+offsetY)*_zoom;
			//计算视窗内target可移动的范围
			var ranges:Array=this.calRange();
			
			xx=ranges[0].valideValue(xx);
			yy=ranges[1].valideValue(yy);	
			//还原_x,_y，让摄像头的_x,_y跟当前真实位置同步，有可能在边界上啊，所以还原
...
//			_y=(lensCenterInTargetParent.y-_offsetY-yy)/_zoom;
		...
		{
			var lensCenterInTargetParent:Point=sceneContainer.parent.globalToLocal(lensCenter);
			_x=(lensCenterInTargetParent.x-_offsetX-sceneContainer.x)/_zoom;
			_y=(lensCenterInTargetParent.y-_offsetY-sceneContainer.y)/_zoom;
		}
		/**
