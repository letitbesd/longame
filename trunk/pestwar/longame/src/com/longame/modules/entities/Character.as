package com.longame.modules.entities
{
	...
	import com.longame.modules.components.fight.FollowComp;
	...
	import com.longame.modules.components.SpeakComp;
	...
	public class Character extends AnimatorEntity implements IHealthEntity, IMovableEntity
	{
		...
		private var speakComp:SpeakComp;
		/**
		 * 说话
		 * @param title:标题
		 * @param content:内容
		...
		public function speak(title:String,content:String=null,showTime:int=3):void
		{
			...
			speakComp.speak(title,content,showTime);
		}
...
//			if(_followingTarget==value) return;
			if(value==null){
				...
				this.state=CharacterState.FOLLOW;
			}
		}
		/**
		...
		{
			//任何处于Idle状态下的都是不忙碌的
			if ((this.state==CharacterState.IDLE_STAND)||(this.state==CharacterState.IDLE_WALK)) return false;
			//任何处于攻击，死亡，逃跑状态的角色都是忙碌的
			if ((this.state==CharacterState.ATTACK)||(this.state==CharacterState.DIE)||(this.state==CharacterState.ESCAPE)) return true;
			//处于follow状态并且follow的对象是敌人，也是忙碌的
			if ((this.state==CharacterState.FOLLOW)&&(this.followingTarget&&(this.followingTarget.faction!=this.faction))) return true;
