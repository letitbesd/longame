 ...
 ******************************************************************************/
package com.longame.managers.input
{
    ...
    public class InputMap //implements ISerializable
    {
        ...
        public function mapKeyToAction(key:InputKey, actionName:String):void
        {
            ...
            if (_keymap[key.keyCode] == null)
            {
                ...
                {
					InputManager.onMouseWheel.add(onMouseWheel);
                }
                else if (key == InputKey.MOUSE_HOVER)
                {
					InputManager.onMouseOver.add(onMouseOver);
					InputManager.onMouseOut.add(onMouseOut);        	
                }
        ...
        public function destroy():void
        {
			...
			InputManager.onMouseOver.remove(onMouseOver);
			InputManager.onMouseOut.remove(onMouseOut);
			InputManager.onMouseWheel.remove(onMouseWheel);
        ...
        private function onMouseMove(event:MouseEvent):void
        {
            ...
            {
                _lastMouseX = Engine.instance.parent.mouseX;
                _lastMouseY = Engine.instance.parent.mouseY;
                _suppressDeltaNextTime = true;
                return;
            }

            if (event.stageX != _lastMouseX || _suppressDeltaNextTime)
                onInputEvent(InputKey.MOUSE_X.keyCode, _suppressDeltaNextTime ? 0 : Engine.instance.parent.mouseX - _lastMouseX);

            if (event.stageY != _lastMouseY || _suppressDeltaNextTime)
                onInputEvent(InputKey.MOUSE_Y.keyCode, _suppressDeltaNextTime ? 0 : Engine.instance.parent.mouseY - _lastMouseY);
        ...
        private function onInputEvent(keyCode:int, value:Number):void
        {
            ...
            if (callback == null)
            {
                Logger.info(this,"onInputEvent", "Got an action for '" + action + "' but no registered callback; ignoring."); 
                return;
            }
