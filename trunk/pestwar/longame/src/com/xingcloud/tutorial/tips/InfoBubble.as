...
//			else pn=new Point((_rect.left+_rect.right)/2,_rect.y);
			if(_rect) pn=new Point((_rect.left+_rect.right)/2,_rect.y);
			else if(target)     pn=this.targetCenterTop;
			else throw new Error("Give me a rectangle or target to show!");
