package com.longame.modules.groups
{
	...
	import com.longame.modules.groups.organizers.LazyGroupLayouter;
	...
	public class DisplayGroup extends DisplayEntity implements IDisplayGroup
	{
		 ...
		 * */
		public function DisplayGroup(id:String=null,autoLayout:Boolean=true)
		{
			super(id);
		...
		override long_internal function get registerGlobalSiganl():Boolean
		{
			return false;
		}
		...
		public function addSimpleLayer(itemIdOrSource:String,id:String=null,index:int=int.MAX_VALUE,state:String=""):SpriteEntity
		{
			...
			if(itemIdOrSource){
				...
				else     entity.source=itemIdOrSource;
			}
        ...
        override protected function activeChild(child:IComponent, refresh:Boolean=true):void
		{
		...
		override protected function deactiveChild(child:IComponent, refresh:Boolean=true):void
		{
		...
		/**
		 * 自己所处单元格发生移动时，让所有的子元素都向外界分发移动事件
		 * todo delete
		 * */
		long_internal function notifyAllChildrenTileMoved(delta:Point):void
		{
			for each(var ch:IComponent in this._activedChildren){
				if(ch is DisplayGroup) (ch as DisplayGroup).notifyAllChildrenTileMoved(delta);
				else if( ch is DisplayEntity) (ch as DisplayEntity).onTileMove.dispatch(ch,delta);
			}
		}
...
//				_sizeInvalidated=true;
		...
		long_internal var _entityMap:GroupMap;
		public function get map():GroupMap
		{
			if(_entityMap==null) {
				_entityMap=new GroupMap(this is IScene);
