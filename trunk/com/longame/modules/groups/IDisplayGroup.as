package com.longame.modules.groups
{
	...
	public interface IDisplayGroup extends IDisplayEntity, IGroup
	{
		 ...
		 * 子显示对象在方格体系里的位置分布图
		 * */
		function get map():GroupMap;
		/**
		 * 是否用GroupLayouter自动排序，默认为true,像很多2d游戏或2.5D游戏中全是没有高度的方格物品，设为false更节省资源
		 * */
		 ...
		 * @param itemIdOrSource: 物品id或一个显示源（图片，swf的地址或显示元件的class名。。。）
		 * @param index: 添加的层级
		 * @param id: 图层的id
		 * @param state:图层在state状态下有效，见IDisplayEntity.add
		 * */
		function addSimpleLayer(itemIdOrSource:String,id:String=null,index:int=int.MAX_VALUE,state:String=""):SpriteEntity;
		/**
