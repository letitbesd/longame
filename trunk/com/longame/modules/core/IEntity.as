package  com.longame.modules.core
{
	...
	/**
	 * <p>A machine is an object that stores all data for some sort of object in the game. This can be anything from
	 * the Hero to a platform, to the health indicator.</p>
	 * 
	 * <p>Machines are made up of cogs. Each cog performs a specific task within the machine. A complicated machine may have many cogs.
	 * Some cogs perform very specific tasks and others can be re-used in multiple machines.</p>
	 * 
	 * <p>Each machine may have many parameters that can be accessed using <code>getParam()</code> and set using <code>setParam</code>.
	 * These parameters are different from native class properties because they exist and are accessible no matter how strictly typed your
	 * machine reference may be. They are also useful because they can be read and written to as XML attributes in your level XML files.</p>
	 * 
	 * @see AbstractMachine
	 * @see com.citrusengine.cogs.ICog
	 */
	public interface IEntity extends  IComponent, IState
	{
		 ...
		 */
		function add(child:IComponent, state:String = ""):void;
