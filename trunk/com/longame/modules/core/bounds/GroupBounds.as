package com.longame.modules.core.bounds
{
	...
	import com.longame.modules.entities.GroupBoundsDebugger;
	...
	public class GroupBounds extends Bounds implements IBounds
	{
		...
		{
			if(child is GroupBoundsDebugger) return false;
		...
		{
			//todo,对于不应该影响边界的物品忽略，IDisplayEntity是否有个属性呢，叫includeInBounds
			if(child is GroupBoundsDebugger) return false;
		...
		{
			if(child is GroupBoundsDebugger) return;
			var edges:Array;
			//左
			edges=edgeChildren[0];
			i=edges.indexOf(child);
			if(child.bounds.left<=_left) if(i==-1) edges.push(child);
			...
			i=edges.indexOf(child);
			if(child.bounds.back<=_back) if(i==-1) edges.push(child);
			else if(i>=0) edges.splice(i,1);
			//前
