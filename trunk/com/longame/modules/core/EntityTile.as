package com.longame.modules.core
{
	...
	public class EntityTile extends BasicTile implements IDestroyable
	{
		...
		protected var _owner:GroupMap;
		
		public function EntityTile(owner:GroupMap,x:int, y:int)
		{
			super(1,x,y,false);
			_owner=owner;
			_walkable=true;
		 ...
		 * 是哪个displayMap下的tile
		 * */
		public function get owner():GroupMap
		{
			return _owner;
		}
		/**
		...
		{
			return SceneManager.getTilePosition(this.x,this.y);
		}
		/**
		 * 获取此tile指定方向上的邻居
		...
		public function addChild(obj:IDisplayEntity):void
		{
			...
			_childrenCount++;
//			if(obj is IDisplayGroup){
//				var subChildren:Vector.<IDisplayEntity>=(obj as IDisplayGroup).getDisplays();
//			}
			if( !obj.walkable)
			{
				this._walkable=false;
			}
//			Logger.info(this,"addChild",obj+" walkable: "+obj.walkable+"owner: "+owner.group+", tile walkable: "+_walkable);
		}
		public function removeChild(obj:IDisplayEntity):void
		{
			...
			_childrenCount--;
			this.updateWalkable();
//			Logger.info(this,"removeChild",obj+" walkable: "+obj.walkable+"owner: "+owner.group+", tile walkable: "+_walkable);
		}
		/**
		 * 方格内是否有显示物品
		 * */
...
//				if(ch.stackable==false)
		 ...
		 * 更新可行走状态
		 * */
		protected function updateWalkable():void
		{
			for each(var ch:IDisplayEntity in _children){
				if(!ch.walkable)
				{
