package com.longame.modules.components
{
	...
	public class AbstractComp implements IComponent
	{
		...
		final public function active(owner:IEntity=null):void
		{
			if (_actived) return;
			_actived=true;
			_owner=owner;
		...
		private function refreshLinks():void
		{
			...
			for each(var meta:* in _metasWithLink){
				var linked:IComponent;
				try{
					linked=this[meta.varName] as IComponent;
				}catch(e:Error){
					throw new Error("The 'Link' metaData belongs only to a IComponent variable!");
				}
				//如果有id，通过id找那个component，否则通过类型查找
				if(meta.hasOwnProperty("id")) linked=_owner.getChild(meta.id as String);
				else         linked=_owner.getChildByType(getDefinitionByName(meta.varClass) as Class);
			}
		}
		/**
		 * 当注册时调用，子类覆盖
