package com.longame.modules.scenes.utils
{
	...
	public class SceneExporter
	{
		...
		{
			var xml:XML=<Scene type={scene.type} tileSize={scene.tileSize}></Scene>;// physics={scene.physicsEnabled}
		...
		public static function fromDisplayGroup(g:DisplayGroup,xml:XML=null):XML
		{
			...
			if(!g.autoLayout) xml.@autoLayout=false;
			xml.@position=g.x+","+g.y+","+g.z;
			var children:Dictionary=g.allChildren;
			var stateChildren:Vector.<IComponent>;
			var child:DisplayEntity;
			var childXml:XML;
			for(var state:String in children){
				stateChildren=children[state];
				for each(child in stateChildren){
		...
		public static function fromDisplayEntity(d:DisplayEntity,xml:XML=null):XML
		{
			...
			if(d.registration!=Registration.CENTER) xml.@registration="topLeft";
