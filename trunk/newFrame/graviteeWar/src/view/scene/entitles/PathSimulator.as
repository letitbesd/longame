...
//			Main.scene.addChild(checkPart);
		...
		{	
			planet.scene.container.addChild(checkPart);
			checkPart.visible =  false;
			var cd:CollisionData=CDK.check(checkPart,planet.container);	
			if(cd){
				_planet=planet;
		...
		private function checkCollisionWithHero(hero:Hero):Boolean
		{
			...
			if(cd&&hero.heroIndex!=this.fireHeroIndex)
			{
				...
				//				 }
				hitHeroIndex=hero.heroIndex;
				_planet=hero.currentPlanet;
				if(checkPart.parent) checkPart.parent.removeChild(checkPart);
				return true;
			}
			else
...
//			checkPart.visible=false;
			checkPart.x=this.x;
			checkPart.y=this.y;
			if(BattleScene.lightnings.length==-1) return false;
			for each (var lightning:Shape in BattleScene.lightnings)
			{
