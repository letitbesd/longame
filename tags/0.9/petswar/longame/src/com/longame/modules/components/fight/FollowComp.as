package com.longame.modules.components.fight
{
	...
	import com.longame.modules.entities.Pet;
	...
	public class FollowComp extends AbstractComp
	{
		...
		private function checkDistance(tile:EntityTile=null):void
		{
			...
			if(dx*dx+dy*dy>theOwner.vision*theOwner.vision){
				//如果跟随的是敌对角色，超出视距则休息
				if(isOpposed()) {
					theOwner.state=CharacterState.IDLE_WALK;
				}
...
//			}else if(dx+dy==1){
				...
				if(isOpposed()){
					theOwner.state=CharacterState.ATTACK;
				}
			}
		}
		...
		{
			if(this.isOpposed()) theOwner.state=CharacterState.IDLE_WALK;
		}
