...
//		public function add1(child:IComponent,...noStates):void
//		{
//			if(!checkChildValidation(child)){
//				throw new Error("You cannot add a child : "+child.toString()+" to "+this.toString());
//			}
//			if(noStates==null || noStates.length==0) return;
//			if (contains(child,true)) return;
//			//先从旧owner中删除，再添加到新的owner
//			if(child.owner!=null) child.owner.remove(child);
//			allChildren.push(child);
//			for each(var state:String in noStates){
//				if (!allStateChildren[state])
//					allStateChildren[state] = new Vector.<IComponent>();
//				(child as AbstractComp).long_internal::initStateInParent(state);
//			}
//			if(noStates.indexOf(_state)!=-1)
//				activeChild(child);
//			if(_onChildAdded) this._onChildAdded.dispatch(child);
//		}
