package com.longame.game.group.component
{
	...
	import com.longame.game.component.AnimatedComp;
	...
	import com.longame.game.scene.util.TileHilightUtil;
	 ...
	 * 一般情况下，只需对移动对象进行操作
	 * */
	public class GroupLayouter extends AnimatedComp
	{
		//需要重新排序的对象
		...
		{
			return this.owner as IDisplayGroup;
		}
		...
		override public function onFrame(deltaTime:Number):void
		{
			...
			maxChildren=displayGroup.numDisplay;
			//将所有需要置顶的对象置顶
			var topChildren:int=map.alwaysInTopChildren.length;
			for(var i:int=0;i<topChildren;i++){
				this.displayGroup.setChildIndex(map.alwaysInTopChildren[i],maxChildren-1);
				//每置顶一个对象，那么其余对象的最大高度少1
				maxChildren--;
			}
		...
		protected function place(target:IDisplayEntity):void
		{
			...
			var maxDepth:int=maxChildren-invalidtatedChildren.length;
			var newIndex:int;
			var resultDepth:int;
			//初始化本次搜索
			initSearch(target);
...
//					if(result0) trace("found child: ",result0.depth);
		...
		private function findChildInCell(tile:EntityTile,target:IDisplayEntity,lowDepth:Boolean=true):IDisplayEntity
		{
			...
			for(var i:int=0;i<max;i++){
				...
				if((ch==target)||(ch.includeInLayout===false)||(ch.alwaysInTop===true)||((invalidtatedChildren.indexOf(ch)>-1)&&(!childVisited[ch]))) {
					continue;
				}
				return ch;
			}	
