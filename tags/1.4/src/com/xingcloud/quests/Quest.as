package  com.xingcloud.quests
{
	 ...
	 * 典型的XML定义
	 * <Quest id="1342" awards="awardId,awardId1" name="" description="" type="" icon="">
	 *   <Steps>
	...
	public class Quest extends Command implements IQuest
	{
		...
		public static function parseFromXML(xml:XML):Quest
		{
			...
			if(xml.hasOwnProperty("@awards")){
				q._awardIds=StringParser.toArray(xml.@awards);
//				q._awardId=xml.@award;
		...
		xingcloud_internal function setAsCompleted():void
		{
			...
			for (var i:int=0;i<_steps.length;i++){
				step=_steps[i];
				step._completedCount=step.count;
			}
		}
		...
		protected var _awardIds:Array;
		public function get awards():Vector.<AwardItemSpec>
		{
			if(_awardIds==null) return null;
			var specs:Vector.<AwardItemSpec>=new Vector.<AwardItemSpec>();
			for each(var aid:String in _awardIds){
				specs.push(ItemDatabase.getItem(aid) as AwardItemSpec);
			}
			return specs;
		}
		 ...
		 * todo，这里有问题
		 * */
		protected function checkAction(act:Action,data:Object):void
		{
